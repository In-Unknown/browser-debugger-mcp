...
**测试策略：**
1. **执行顺序**：必须先打开页面检查登录状态，根据检查结果决定后续流程
2. **状态判断**：
   - 如果首次打开即显示已登录状态，直接测试持久化功能
   - 如果首次打开显示未登录状态，必须启动人工登录流程
3. **人工登录流程**：
   - 必须调用AskUserQuestion工具提问，提供两个选项：
     1. "我已完成登录操作"
     2. "我点错了，需要重新登录"
   - 如果用户选择"点错了"，必须再次调用AskUserQuestion工具，直到用户选择"已完成登录操作"
   - 等待用户完成登录操作，给用户足够的时间进行登录
4. **工具依赖**：
   - 测试必须使用AskUserQuestion工具进行人工登录，最优先，如果你能提问必须执行提问。
   - 如果无法调用AskUserQuestion工具，应先暂时立即终止测试，要求上级AI调用AskUserQuestion工具，等AI提问并完成之后要求他重新调用你并继续操作.
5. **完整验证流程**：登录完成后，必须关闭浏览器，然后重新打开浏览器对应的页面，检查登录状态是否保持
6. **多网站覆盖**：必须测试所有列出的网站（站酷、花瓣、GitHub等），确保持久化功能的通用性

**测试网站列表：**
1. 站酷：https://www.zcool.com.cn
2. 花瓣：https://huaban.com
3. GitHub：https://github.com
4. 可选：其他支持登录的网站（如掘金、知乎等）
...
6. 必须使用AskUserQuestion工具进行人工登录操作，对于每个网站需要修改问题文本中的网站名称，并确保测试所有列出的网站（站酷、花瓣、GitHub等）
7. **强制工具依赖**：如果测试过程中无法调用AskUserQuestion工具，必须立即终止测试，并向用户报告需要上级AI调用AskUserQuestion工具来完成测试
